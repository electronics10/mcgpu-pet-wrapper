# A Python Interface for Monte Carlo PET Simulation
...
- Only two materials, namely air and water, are used in the example.
...
```json
{
  ...
  "mcgpu": {
    ...
    "materials": [
      ...
      "materials/adipose_ICRP110_5-515keV.mcgpu.gz",
      "materials/spongiosa_ICRP110_5-515keV.mcgpu.gz"
    ]
  }
}
```
...
As for the materials, one can state up to 15 materials (1~15), choosing from the materials folder. The limitation is from the MCGPU-PET original code. (The materials provided by MCGPU-PET are generated by post processing materials from PENELOPE 2006.) At 511keV, Compton interaction dominates (photoelectric and Rayleigh are ~1–2% combined). Compton cross-section per electron is nearly the same for all elements, so what matters is electrons-per-gram. Scaling density can compensate for a different Z/A (you'd just pick a density that gives the right electron density). So Z/A differences are largely fakeable by density adjustment. The approximation breaks only when the photoelectric($\sim Z^{4-5}$)/Rayleigh($\sim Z^2$) fraction stops being negligible, which happens when effective Z gets high. Overall, in the template, I choose to include air, water, adipose, and spongiosa. (Further materials such as coritcal bone and metal are troublesome to generate, so it's left out.)

Finally,we write a module `config.py` for loading the json file, checking it for internal contradictions, and computing some derived numbers that the rest of the package needs. If the config is wrong, we want to fail here, loudly, with some helpful messages.
